export const technicalAnalysis = {
  ...
  calculateMACD(prices, options = {}) {
    ...
    if(!prices || prices.length < Math.max(fastPeriod, slowPeriod) + signalPeriod) return {macdLine:[],signalLine:[],histogram:[],previousHistogram:null,lastHistogram:null};
    ...
    for(let i=0; i<ema12.length; i++) macdLine[i] = ema12[i] - (i < ema26.length ? ema26[i] : 0);
    const signalLine = this.calculateEMA(macdLine, signalPeriod);
    const histogram = new Array(macdLine.length);
    for(let i=0; i<macdLine.length; i++) histogram[i] = macdLine[i] - (i < signalLine.length ? signalLine[i] : 0);
    const result = {macdLine,signalLine,histogram,previousHistogram:histogram.length>1 ? histogram[histogram.length-2] : null,lastHistogram:histogram.length>0 ? histogram[histogram.length-1] : null};
    this._cache.set(cacheKey, result);
    return result;
  },
  ...
  calculateBollingerBands(prices, period = 20, stdDev = 2) {
    ...
    const result = {upper:middle + (standardDeviation * stdDev),middle,lower:middle - (standardDeviation * stdDev)};
    this._cache.set(cacheKey, result);
    return result;
  },
  ...
  calculateIndicators(prices, options = {}) {
    ...
    const macdOptions = {fastPeriod:options.fastPeriod || 12,slowPeriod:options.slowPeriod || 26,signalPeriod:options.signalPeriod || 9};
  ...
  analyzeAll(prices, volumes, options = {}) {
    ...
    if(volumes?.length === prices.length && volumes.length > 0) {
      ...
      volumeAnalysis = {currentVolume:volumes[volumes.length - 1],averageVolume:volSMA[volSMA.length - 1],relativeVolume,increasing:volumes.length > 1 && volumes[volumes.length - 1] > volumes[volumes.length - 2]};
    }
    ...
    return {
      ...
      priceAction:{
        ...
        percentChange:prices.length > 1 ? ((prices[prices.length - 1] - prices[prices.length - 2]) / prices[prices.length - 2]) * 100 : 0
      }
    };
  },
