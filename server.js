...
const botConfig = {
  ...tradingConfig, security: securityConfig, api: apiConfig,
  performance: {
    tokenConcurrency: 5, enableAutomaticRestarts: true,
    memoryThreshold: 1536, memoryCheckInterval: 300000
  }
};
...
app.use((req, res, next) => {
  ...
  res.on('finish', () => {
    ...
    if (responseBody && JSON.stringify(responseBody).length > 1000) {
      responseBody = { 
        type: typeof responseBody, size: JSON.stringify(responseBody).length,
...
app.get('/api/verify-auth', authenticateJWT, (req, res) => res.json({
  authenticated: true, user: { username: req.user.username, role: req.user.role }
}));

app.get('/api/health', cacheMiddleware(30000), (req, res) => {
  const cachedHealth = healthCheckCache.get('health');
  if (cachedHealth && Date.now() - cachedHealth.timestamp < 5000) return res.json(cachedHealth.data);
  const healthData = {
...
app.post('/api/start', authenticateJWT, authorizeRoles('admin'), csrfProtection, async (req, res) => {
  ...
  if (tradingBot.isRunning) return res.status(400).json({ error: 'Le bot est déjà en cours d\'exécution' });
  try {
    ...
    } else throw new Error('Échec du démarrage du bot');
  } catch (error) {
...
app.post('/api/stop', authenticateJWT, authorizeRoles('admin'), csrfProtection, async (req, res) => {
  ...
  if (!tradingBot.isRunning) return res.status(400).json({ error: 'Le bot n\'est pas en cours d\'exécution' });
  try {
    const report = await tradingBot.stop();
    apiCache.delete('/api/status');
    logService.logSecurityEvent('bot_stop_success', { ip: req.ip, user: req.user.username });
    res.json({ 
      success: true, message: 'Bot arrêté avec succès',
      report: {
...
app.post('/api/pause', authenticateJWT, authorizeRoles('admin'), csrfProtection, async (req, res) => {
  ...
  try {
    ...
    } else throw new Error('Échec de la mise en pause du bot');
  } catch (error) {
...
app.post('/api/resume', authenticateJWT, authorizeRoles('admin'), csrfProtection, async (req, res) => {
  ...
  try {
    ...
    } else throw new Error('Échec de la reprise du bot');
  } catch (error) {
...
app.post('/api/simulation', authenticateJWT, csrfProtection, validationRules.simulation, validate, async (req, res) => {
  ...
  if (!startDate || !endDate) return res.status(400).json({ error: 'Les dates de début et de fin sont requises' });
  try {
    const simulationResults = await tradingBot.runSimulation(
      new Date(startDate), new Date(endDate), parameters
    );
...
app.post('/api/config', authenticateJWT, authorizeRoles('admin'), csrfProtection, async (req, res) => {
  ...
  try {
    ...
    } else throw new Error(result.error || 'Échec de la mise à jour de la configuration');
  } catch (error) {
...
io.on('connection', (socket) => {
  ...
  try {
    ...
    socket.emit('bot_status', { isRunning: tradingBot.isRunning, isPaused: tradingBot.isPaused });
...
const broadcastUpdates = async (socket = null) => {
  try {
    if (tradingBot.isRunning) {
      ...
      const updateData = { 
        report, recentTrades, timestamp: new Date().toISOString(),
        status: {
      ...
      if (socket) socket.emit('bot_update', updateData);
      else io.emit('bot_update', updateData);
    }
  } catch (error) {
